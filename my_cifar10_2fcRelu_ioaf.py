...
from config import FCRELU_PRETRAINED_CIFAR10, PLOT_FOLDER
...
input_shape = (3, 32, 32)
model_folder = FCRELU_PRETRAINED_CIFAR10
...
for name, n in zip(clf_names, expansions):
    if not (model_folder / name).exists():
        net = Expandable2LinearReLu(expansion=n, mnist=False, colored=True, out_classes=10)
...
plot_performance(clfs, tr_dataset, ts_dataset, CArray(parameters), "Network parameters",
                             title="Accuracy of 2 layered Relu network by varying numbers of parameters", savefig=str(PLOT_FOLDER / "cifar10_fcrelu_base_performance"))
indicators_clf = {'epsilons': epsilons, 'overparam' : {}}
if not (FCRELU_PRETRAINED_CIFAR10 / "indicators.pkl").exists():
    for p, clf in zip(parameters, clfs):
        indicators_eps = {'indicators': {}, 'robacc': {}}
        for eps in epsilons:
            print(f"Attack 2L-Relu with {p} params with PGDL2 eps={eps}")
            indicators_samples = []
            attack_success = []
    ...
    with open(str(FCRELU_PRETRAINED_CIFAR10 / "indicators.pkl"), 'wb') as h:
