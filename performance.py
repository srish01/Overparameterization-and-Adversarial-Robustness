...
def performance_score_model(classifier: CClassifier, tr_dataset: CDataset, ts_dataset: CDataset):
    ...
    tr_error = 1 - CMetricAccuracy().performance_score(tr_dataset.Y, tr_y_pred)
    ts_error = 1 - CMetricAccuracy().performance_score(ts_dataset.Y, ts_y_pred)
...
def test_models(classifiers, tr_dataset, ts_dataset):
    ...
    for c in classifiers:
        tr, ts = performance_score_model(c, tr_dataset, ts_dataset)
        ts_errors.append(ts)
        tr_errors.append(tr)
