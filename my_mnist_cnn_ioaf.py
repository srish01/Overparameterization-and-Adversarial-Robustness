...
from config import REG_TINYNET_PRETRAINED_MNIST, PLOT_FOLDER
from ioaf import compute_indicators
from performance import plot_performance
from tinynet import ExpandableTinyNet
...
input_shape = (1, 28, 28)
model_folder = REG_TINYNET_PRETRAINED_MNIST
...
for name, n in zip(clf_names, expansions):
    if not (model_folder / name).exists():
        net = ExpandableTinyNet(expansion=n, mnist=True, colored=False, out_classes=len(DIGITS))
...
plot_performance(clfs, tr_dataset, ts_dataset, CArray(parameters), "Network parameters",
                             title="Accuracy of CNN by varying numbers of parameters",
                             savefig= str(PLOT_FOLDER / "mnist_cnn_base_performance"))
indicators_clf = {'epsilons': epsilons, 'overparam' : {}}
if not (REG_TINYNET_PRETRAINED_MNIST / "indicators.pkl").exists():
    ...
    with open(str(REG_TINYNET_PRETRAINED_MNIST / "indicators.pkl"), 'wb') as h:
