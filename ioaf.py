...
def compute_indicators(attack, x, y, clf, transfer_clf=None, is_patched=False) -> pd.DataFrame:
    """
    ...
    """
    # run attack
    y_adv, atk_scores, adv_ds, f_opt = attack.run(x, y)

    # get information required for computing the indicators
    y_real = CArray(attack._y0)
    transfer_scores = transfer_clf.predict(adv_ds.X) if transfer_clf is not None else None
    y_target = attack.y_target
